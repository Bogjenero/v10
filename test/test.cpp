...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_06)
		 {
			...
			vector<int>::iterator it = find_if(v.begin(), v.end(), [](int n) {
				for (int i = 2; i <= n / 2; i++)
					if (n % i == 0)
						return false;
				return true; });
		...
		TEST_METHOD(test_07b)
		{
			...
			replace_if(s.begin(), s.end(), [](char s1) {
				std::string s2 = "aeiou";
				for (char ch : s2) {
					if (ch == s1) {
						return true;
		...
		TEST_METHOD(test_08b)
		{
			...
			s.erase(remove_if(s.begin(), s.end(), [](char s1) {
				std::string s2 = "aeiou";
				for (char ch : s2) {
					if (ch == s1) {
						return true;
		...
		TEST_METHOD(test_09)
		{
			...
			struct cmp {
				bool operator() (const exam& e, const exam& e2) {
					if (e.grade == e2.grade)
						return e.points > e2.points;
					return e.grade > e2.grade;
				}
			};
